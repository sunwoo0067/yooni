-- 마켓플레이스 통합 관리를 위한 데이터베이스 테이블 생성
...
CREATE TABLE IF NOT EXISTS marketplace_health_status (
    ...
    error_message TEXT,
    INDEX idx_marketplace_checked (marketplace, checked_at)
);
...
CREATE TABLE IF NOT EXISTS marketplace_metrics (
    ...
    INDEX idx_marketplace_metrics (marketplace, collected_at)
);
...
CREATE TABLE IF NOT EXISTS marketplace_request_logs (
    ...
    error_message TEXT,
    INDEX idx_marketplace_logs (marketplace, created_at),
    INDEX idx_endpoint_logs (endpoint, created_at)
);
...
CREATE TABLE IF NOT EXISTS marketplace_accounts (
    ...
    INDEX idx_marketplace_accounts (marketplace, is_active)
);
...
CREATE INDEX IF NOT EXISTS idx_accounts_active ON marketplace_accounts(marketplace, is_active, is_default);

-- 파티셔닝 (대용량 로그 데이터 관리)
-- 일별 파티션으로 로그 테이블 관리 (PostgreSQL 10+ 필요)
-- CREATE TABLE marketplace_request_logs_partition PARTITION OF marketplace_request_logs 
