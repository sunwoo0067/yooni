"""
쿠팡 출고지 수정(UPDATE) API 예제
- HMAC 인증 포함
- .env에서 accesskey, secretkey, vendorId 자동 로딩
- outboundShippingPlaceCode, remoteInfoId 등은 예시값 사용
- 실제 요청은 주석 처리(테스트용)
"""
import os
import time
...
if __name__ == "__main__":
    method = "PUT"
    outboundShippingPlaceCode = 123456  # 예시값, 실제 값으로 교체 필요
    path = f"/v2/providers/openapi/apis/api/v4/vendors/{VENDOR_ID}/outboundShippingCenters/{outboundShippingPlaceCode}"
    ...
    body_dict = {
        ...
        "remoteInfos": [
            {
                "remoteInfoId": 176476,  # 예시값, 실제 값으로 교체 필요
                "deliveryCode": "DONGBU",
                "jeju": 0,
                "notJeju": 0,
                "usable": True
