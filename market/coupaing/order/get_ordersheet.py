...
BASE_PATH = f"/v2/providers/openapi/apis/api/v4/vendors/{VENDOR_ID}/ordersheets"
...
def fetch_and_update_order(shipment_box_id):
    print(f"========== shipmentBoxId: {shipment_box_id} 주문 단건 조회를 시작합니다. ==========")
    
    path = f"{BASE_PATH}/{shipment_box_id}"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("조회할 shipmentBoxId를 입력해주세요.")
        print("사용법: python3 get_ordersheet.py <shipmentBoxId>")
        sys.exit(1)

    shipment_box_id_to_fetch = sys.argv[1]
    fetch_and_update_order(shipment_box_id_to_fetch)
