...
def complete_shipment(vendor_id, receipt_id, delivery_company_code, invoice_number):
    """
    이미출고 처리
    :param vendor_id: 판매자 ID
    :param receipt_id: 접수 ID
    :param delivery_company_code: 택배사 코드
    :param invoice_number: 송장번호
    :return: API 응답 JSON
    """
    print(f"========== 접수번호 {receipt_id} 이미출고 처리를 시작합니다. ==========")
    
    path = f"/v2/providers/openapi/apis/api/v4/vendors/{vendor_id}/returnRequests/{receipt_id}/completedShipment"
...
if __name__ == "__main__":
    ...
    example_receipt_id = 12346897
    example_delivery_company_code = "KDEXP"
    example_invoice_number = "70432743577"

    print("\n### 예시 이미출고 처리 실행 ###")
    print("주의: main 블록의 예시 데이터는 실제 데이터가 아니므로, API 호출 시 실패할 수 있습니다.")
    print("실제 사용 시에는 예시 변수들을 유효한 데이터로 채워야 합니다.")
    
    # 아래 라인의 주석을 해제하면 예시 데이터로 API 호출을 시도합니다.
    # complete_shipment(vendor_id_from_env, example_receipt_id, example_delivery_company_code, example_invoice_number)
    
    print("\n스크립트 사용 안내:")
    print("1. 이 파일(complete_shipment_on_return_request.py)을 직접 수정하여 변수에 실제 데이터를 넣고 실행할 수 있습니다.")
    print("2. 다른 Python 스크립트에서 `from market.coupaing.order.complete_shipment_on_return_request import complete_shipment`로 함수를 임포트하여 사용할 수 있습니다.")
