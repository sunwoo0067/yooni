...
def upload_invoice(vendor_id, invoice_dtos):
    """
    송장 정보를 업로드하여 주문 상태를 '배송지시'로 변경합니다.
    :param vendor_id: 판매자 ID
    :param invoice_dtos: 송장 정보 DTO 목록 (list of dicts)
    :return: API 응답 JSON
    """
    print(f"========== vendorId: {vendor_id} 송장 업로드를 시작합니다. ==========")
...
if __name__ == "__main__":
    ...
    # TODO: 아래 예시 데이터를 실제 업로드할 송장 정보로 교체해야 합니다.
    # 이 데이터는 데이터베이스나 다른 소스에서 가져와야 합니다.
    example_invoice_dtos = [
        {
            "shipmentBoxId": 123456789012345678, # 테스트용 ID, 실제 ID로 변경 필요
            "orderId": 4000019469460,           # 테스트용 ID, 실제 ID로 변경 필요
            "vendorItemId": 3823839899,         # 테스트용 ID, 실제 ID로 변경 필요
            "deliveryCompanyCode": "KDEXP",     # 실제 택배사 코드로 변경 필요
            "invoiceNumber": "123456789012",    # 실제 송장번호로 변경 필요
            "splitShipping": False,
    ...
    print("실제 사용 시에는 `example_invoice_dtos`를 유효한 데이터로 채워야 합니다.")
    
    # 아래 라인의 주석을 해제하면 예시 데이터로 API 호출을 시도합니다.
    # upload_invoice(vendor_id_from_env, example_invoice_dtos)
    
    print("\n스크립트 사용 안내:")
    print("1. 이 파일(update_order_shipping_info.py)을 직접 수정하여 `example_invoice_dtos` 변수에 실제 업로드할 데이터를 넣고 실행할 수 있습니다.")
    print("2. 다른 Python 스크립트에서 `from market.coupaing.order.update_order_shipping_info import upload_invoice`로 함수를 임포트하여 사용할 수 있습니다.")
