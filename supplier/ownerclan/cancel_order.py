# -*- coding: utf-8 -*-
"""
Ownerclan의 '주문 취소' API를 호출하는 스크립트입니다.
결제 완료(paid) 상태인 주문을 취소합니다.
"""
...
# 주문 취소를 위한 GraphQL mutation 쿼리입니다.
...
def cancel_order(order_key: str) -> dict:
    """
    주어진 주문 key에 해당하는 주문을 취소합니다.

    Args:
        order_key: 취소할 주문의 코드.
    ...
    print(f"{GRAPHQL_URL} 에 주문 취소를 요청합니다...")
...
def main():
    ...
    스크립트의 메인 실행 함수입니다.
    """
    parser = argparse.ArgumentParser(description="Ownerclan 주문을 취소합니다.")
    parser.add_argument("key", type=str, help="취소할 주문의 key입니다.")
    args = parser.parse_args()

    try:
        result = cancel_order(args.key)
        print("\n--- API 응답 ---")
